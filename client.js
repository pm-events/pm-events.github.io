...
document.getElementById("rumors").onclick = () => {
  ...
  setInterval(() => {
    ...
    document.getElementById("rumorstitles").innerHTML = nextEvents.length >= 1 ? "<b style=\"\">" + nextEvents.map(e => e.name.map(a => a.replace("[grid]", "<img src=\"./grid.png\"/>").replace("[ex]", "<img src=\"./ex.png\"/>").replace("[gridEX]", "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>"))).flat().join("<br>") + "</b>" : "No rumors yet.";
    ...
    document.getElementById("futurerumors").innerHTML = nextEvents.length >= 1 ? futureEvents.map(e => e.name.map(n => `${n} - <span style="color:#ffc361;">${new Date(e.date + " 06:00:00 UTC").toDateString()}</span>`.replace("[grid]", "<img src=\"./grid.png\"/>").replace("[ex]", "<img src=\"./ex.png\"/>").replace("[gridEX]", "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>"))).flat().join("<br>") : null;
  }, 1000);
